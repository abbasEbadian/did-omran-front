...
function Score() {
  const [rating, setRating] = useState(0); // امتیاز فعلی
  const [hoverRating, setHoverRating] = useState(0); // امتیاز هنگام hover
  ...
  return (
    <div className="flex items-center gap-2 flex-col">
      ...
      <span className="text-secondary900 text-xs">
        ({rating > 0 ? `${rating.toFixed(1)} امتیاز` : "بدون امتیاز"}، 21 نظر)
      </span>
    </div>
  );
}
